...
import { Building2, Printer, FileSpreadsheet, Calendar, Search, ArrowDownLeft, ArrowUpRight } from 'lucide-react';
...
export default function VendorStatement({ report, vendors, filters, company }: Props) {
    ...
    const handleFilter = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/reports/vendor-statement', form, { preserveState: true });
    };
    ...
    return (
        <div className="flex flex-col gap-6 p-6 max-w-6xl mx-auto w-full">
            <Head title={isRtl ? 'كشف حساب مورد تفصيلي' : 'Vendor Statement of Account'} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
                        ...
                        <span>{isRtl ? 'كشف حساب مورد تفصيلي' : 'Vendor Statement of Account'}</span>
                    </h1>
                    <p className="text-sm text-neutral-500 mt-1">
                        {isRtl
                            ? 'استخراج فواتير الشراء وسندات الصرف والرصيد المستحق للمورد خلال فترة محددة'
                            : 'Detailed vendor bills, payments, and running liability balance over time'}
                    </p>
                </div>
                ...
                <div className="flex items-center gap-2 self-start sm:self-auto">
                    {report && report.vendor && (
                        <Button asChild variant="outline" size="sm" className="gap-1.5 border-emerald-300 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-400">
                            <a href={`/reports/vendor-statement/export?vendor_id=${form.vendor_id}&start_date=${form.start_date}&end_date=${form.end_date}`}>
            ...
            <div className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 print:hidden">
                <form onSubmit={handleFilter} className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
                    <div className="flex flex-col gap-1.5 sm:col-span-2">
                        <Label htmlFor="vnd_select">{isRtl ? 'المورد المعتمد' : 'Select Vendor'}</Label>
                        <select
                            id="vnd_select"
                        ...
                        >
                            ...
                            {vendors.map((v) => (
                                <option key={v.id} value={v.id}>
                                    {v.name_ar ? `${v.name_ar} (${v.name})` : v.name} {v.tax_id ? `| ${v.tax_id}` : ''}
                                </option>
                            ))}
                        </select>
                    </div>
            ...
            {report && report.vendor ? (
                <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 p-8 shadow-sm flex flex-col gap-6 print:border-none print:shadow-none print:p-0">
                    ...
                    <div className="flex justify-between items-start border-b border-neutral-200 dark:border-neutral-800 pb-6">
                        ...
                        <div className="text-right">
                            <span className="inline-block px-3 py-1 bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 font-bold rounded-md text-sm border border-emerald-200 dark:border-emerald-800">
                                {isRtl ? 'كشف حساب مورد معتمد' : 'Vendor Statement of Account'}
                            </span>
                    ...
                    <div className="bg-neutral-50 dark:bg-neutral-800/40 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row justify-between gap-4 text-sm">
                        <div>
                            <span className="text-xs font-semibold text-neutral-400 uppercase">{isRtl ? 'بيانات المورد' : 'Vendor Details'}</span>
                            <h3 className="text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
                                {report.vendor.name_ar ? `${report.vendor.name_ar} - ${report.vendor.name}` : report.vendor.name}
                            </h3>
                            {report.vendor.phone && (
                                <p className="text-xs text-neutral-500 mt-1">{isRtl ? 'الهاتف' : 'Phone'}: {report.vendor.phone}</p>
                            )}
                        </div>
                        {report.vendor.tax_id && (
                            <div className="sm:text-right">
                                <span className="text-xs font-semibold text-neutral-400 uppercase">{isRtl ? 'الرقم الضريبي للمورد' : 'Vendor Tax ID'}</span>
                                <p className="font-mono text-xs font-bold text-neutral-800 dark:text-neutral-200 mt-0.5">{report.vendor.tax_id}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 print:gap-2">
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 p-3.5 bg-neutral-50/50 dark:bg-neutral-900/50">
                            <span className="text-xs text-neutral-500">{isRtl ? 'رصيد افتتاحي سابق للمورد' : 'Opening Liability'}</span>
                        ...
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 p-3.5 bg-amber-50/30 dark:bg-amber-950/20">
                            <span className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1">
                                ...
                                <span>{isRtl ? 'فواتير المورد (دائن/التزام)' : 'Total Bills (Credit)'}</span>
                            </span>
                        ...
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 p-3.5 bg-emerald-50/30 dark:bg-emerald-950/20">
                            <span className="text-xs text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                                ...
                                <span>{isRtl ? 'سندات الصرف (مدين/سداد)' : 'Payments Made (Debit)'}</span>
                            </span>
                        ...
                        <div className="rounded-xl border border-emerald-200 dark:border-emerald-900/60 p-3.5 bg-emerald-50/50 dark:bg-emerald-950/40">
                            <span className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold">{isRtl ? 'صافي الرصيد المستحق للمورد' : 'Closing Liability Balance'}</span>
                            <div className="text-lg font-mono font-black text-emerald-900 dark:text-emerald-200 mt-1">
                        ...
                        <table className="w-full text-xs text-left">
                            <thead className="bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 font-semibold border-b border-neutral-200 dark:border-neutral-800">
                                <tr>
                                    ...
                                    <th className="py-2.5 px-3">{isRtl ? 'نوع المستند' : 'Type'}</th>
                                    <th className="py-2.5 px-3">{isRtl ? 'رقم السند / المرجع' : 'Reference'}</th>
                                    <th className="py-2.5 px-3 text-right">{isRtl ? 'مدين (سداد -)' : 'Debit (Payments -)'}</th>
                                    <th className="py-2.5 px-3 text-right">{isRtl ? 'دائن (فواتير +)' : 'Credit (Bills +)'}</th>
                                    <th className="py-2.5 px-3 text-right font-bold">{isRtl ? 'الرصيد التراكمي للمورد' : 'Running Liability'}</th>
                            ...
                            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800 font-mono">
                                ...
                                {report.transactions.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="py-8 text-center text-neutral-400 font-sans">
                                            {isRtl ? 'لا توجد حركات مالية مسجلة للمورد خلال هذه الفترة' : 'No transactions recorded during this period.'}
                                        </td>
                                    </tr>
                                ) : (
                                    report.transactions.map((tx, idx) => (
                                        <tr key={idx} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30">
                                            ...
                                            <td className="py-2 px-3 text-right text-amber-800 dark:text-amber-200">
                                                {tx.credit > 0 ? Number(tx.credit).toLocaleString('en-US', { minimumFractionDigits: 2 }) : '-'}
                                            </td>
                                            <td className="py-2 px-3 text-right font-bold text-emerald-950 dark:text-emerald-200 bg-neutral-50/30 dark:bg-neutral-800/20">
                                ...
                                <tr className="bg-neutral-100 dark:bg-neutral-800 font-bold border-t-2 border-neutral-300 dark:border-neutral-700 text-neutral-900 dark:text-neutral-100">
                                    <td colSpan={3} className="py-2.5 px-3 font-sans text-right">
                                        {isRtl ? 'إجمالي حركات الفترة والرصيد الختامي' : 'Total Movements & Closing Liability'}
                                    </td>
                                    ...
                                    <td className="py-2.5 px-3 text-right text-amber-800 dark:text-amber-300">
                                        {Number(report.total_credit).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                    </td>
                                    <td className="py-2.5 px-3 text-right font-black text-emerald-700 dark:text-emerald-400 text-sm">
                    ...
                    <div className="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-800 grid grid-cols-3 gap-8 text-center text-xs">
                        ...
                        <div>
                            <p className="font-semibold text-neutral-700 dark:text-neutral-300">{isRtl ? 'ختم ومصادقة المورد' : 'Vendor Acknowledgement'}</p>
                            <div className="h-14 border-b border-dashed border-neutral-300 dark:border-neutral-700 mt-2"></div>
                            <p className="text-[11px] text-neutral-400 mt-1">{isRtl ? 'المصادقة على الرصيد الدائن' : 'Balance Confirmation'}</p>
                        </div>
                    </div>
                </div>
            ) : (
                <div className="rounded-xl border border-neutral-200 bg-white p-12 text-center text-neutral-500 dark:border-neutral-800 dark:bg-neutral-900">
                    ...
                    <p>{isRtl ? 'يرجى اختيار المورد وتحديد الفترة لاستعراض كشف الحساب' : 'Please select a vendor and period to generate the statement.'}</p>
                </div>
            )}
        </div>
    );
