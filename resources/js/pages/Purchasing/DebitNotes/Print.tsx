...
export default function DebitNotesPrint({
    debitNote,
    company,
    qrCodeDataUri,
    amountInWords,
    ...
    return (
        <div className="min-h-screen bg-neutral-100 dark:bg-neutral-950 py-8 px-4 sm:px-6 print:p-0 print:bg-white text-neutral-900">
            <Head title={`${debitNote.debit_note_number} - ${isRtl ? 'إشعار مدين رسمي للمورد' : 'Tax Debit Note'}`} />
            ...
            <div className="max-w-4xl mx-auto mb-6 flex items-center justify-between print:hidden">
                <div className="flex items-center gap-2">
                    <Button onClick={() => window.print()} className="bg-indigo-600 hover:bg-indigo-700 text-white gap-2">
                        ...
                        {isRtl ? 'طباعة الإشعار المدين' : 'Print Debit Note'}
                    </Button>
                ...
                <div className="text-xs text-neutral-500">
                    {isRtl ? 'إشعار ضريبي مدين صادر للمورد وفق اشتراطات هيئة الزكاة والضريبة والجمارك' : 'Tax Debit Note to Vendor'}
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto bg-white p-8 sm:p-12 rounded-lg shadow-sm border border-neutral-200 print:border-none print:shadow-none print:p-0 print:m-0 print:w-full">
                ...
                <div className="flex justify-between items-start border-b-2 border-neutral-900 pb-6 mb-6">
                    ...
                    <div className="text-center flex flex-col items-center">
                        <div className="inline-block border-2 border-indigo-600 text-indigo-600 px-4 py-1 font-black text-base sm:text-lg uppercase tracking-wider rounded">
                            {isRtl ? 'إشعار مدين ضريبي' : 'TAX DEBIT NOTE'}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-6 bg-neutral-50 p-4 rounded border border-neutral-200 text-xs mb-6">
                    <div className="space-y-1.5 text-start">
                        ...
                        <div className="font-semibold text-sm text-neutral-900">{debitNote.vendor?.name}</div>
                        {debitNote.vendor?.tax_id && (
                            <div>{isRtl ? 'الرقم الضريبي للمورد:' : 'Vendor VAT ID:'} <span className="font-mono">{debitNote.vendor.tax_id}</span></div>
                        )}
                        {debitNote.vendor?.phone && (
                            <div>{isRtl ? 'الهاتف:' : 'Phone:'} {debitNote.vendor.phone}</div>
                        )}
                        {debitNote.vendor?.address && (
                            <div>{isRtl ? 'العنوان:' : 'Address:'} {debitNote.vendor.address}</div>
                        )}
                    </div>

                    <div className="space-y-1.5 text-start">
                        <div className="font-bold text-neutral-700 uppercase tracking-wider border-b border-neutral-200 pb-1 mb-1">
                            {isRtl ? 'تفاصيل الإشعار المدين' : 'Debit Note Details'}
                        </div>
                        <div>{isRtl ? 'رقم الإشعار:' : 'Note #:'} <span className="font-mono font-bold">{debitNote.debit_note_number}</span></div>
                        <div>{isRtl ? 'تاريخ التحرير:' : 'Issue Date:'} <span className="font-mono">{debitNote.date}</span></div>
                        <div>{isRtl ? 'سبب الإرجاع:' : 'Reason:'} <span>{debitNote.reason || (isRtl ? 'مردودات مشتريات' : 'Purchase return')}</span></div>
                        <div>{isRtl ? 'الفرع:' : 'Branch:'} <span>{debitNote.branch?.name || (isRtl ? 'المركز الرئيسي' : 'HQ')}</span></div>
                    </div>
                </div>
                ...
                <table className="w-full text-xs text-start mb-6 border border-neutral-300">
                    <thead className="bg-neutral-100 border-b border-neutral-300 font-bold text-neutral-700">
                        <tr>
                            ...
                            <th className="py-2.5 px-3 text-end border-e border-neutral-300 w-28">{isRtl ? 'تكلفة الوحدة' : 'Unit Cost'}</th>
                ...
                <div className="grid grid-cols-2 gap-4 items-start mb-8">
                    ...
                    <div className="border border-neutral-300 rounded overflow-hidden text-xs">
                        <div className="flex justify-between py-2 px-3 border-b border-neutral-200">
                            ...
                            <span className="font-mono font-semibold">{parseFloat(debitNote.subtotal).toFixed(2)} SAR</span>
                        </div>
                        <div className="flex justify-between py-2 px-3 border-b border-neutral-200">
                            <span className="text-neutral-600">{isRtl ? 'ضريبة القيمة المضافة المعكوسة (15%):' : 'Reversed VAT (15%):'}</span>
                            <span className="font-mono font-semibold">{parseFloat(debitNote.tax_amount).toFixed(2)} SAR</span>
                        </div>
                        <div className="flex justify-between py-2.5 px-3 bg-neutral-100 font-bold text-sm">
                            <span className="text-neutral-900">{isRtl ? 'إجمالي الخصم من ذمة المورد:' : 'Total Debited to AP:'}</span>
                            <span className="font-mono text-indigo-700">{parseFloat(debitNote.total).toFixed(2)} SAR</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-6 pt-6 border-t-2 border-neutral-200 text-center text-xs">
                    <div>
                        <div className="text-neutral-500 mb-10">{isRtl ? 'مسؤول المشتريات / المستودع' : 'Purchasing / Warehouse Officer'}</div>
                    ...
                    <div>
                        <div className="text-neutral-500 mb-10">{isRtl ? 'استلام وتأكيد المورد' : 'Vendor Acknowledgment'}</div>
                        <div className="border-t border-neutral-400 pt-1 font-medium">{isRtl ? 'الاسم / التوقيع والختم' : 'Name / Stamp'}</div>
                    </div>
                </div>
            </div>
        </div>
