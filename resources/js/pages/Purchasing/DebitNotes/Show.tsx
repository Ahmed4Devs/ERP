...
import { ArrowLeft, ArrowRight, Printer, CheckCircle2, Clock, RotateCcw, ShieldCheck, Building2, User, Landmark } from 'lucide-react';
...
export default function DebitNotesShow({ debitNote }: Props) {
    const { isRtl } = useTranslation();

    const handlePost = () => {
        if (confirm(isRtl ? 'هل أنت متأكد من ترحيل هذا الإشعار المدين وتخفيض ذمة المورد وعكس ضريبة المدخلات وإخراج البضاعة من المخزن؟' : 'Are you sure you want to post this debit note? This will debit AP, credit input VAT and issue items from warehouse.')) {
    ...
    return (
        <div className="flex flex-col gap-6 p-6 max-w-5xl mx-auto w-full">
            <Head title={`${isRtl ? 'إشعار مدين' : 'Debit Note'} ${debitNote.debit_note_number}`} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-neutral-500 mt-0.5">
                            {isRtl ? 'تاريخ التحرير:' : 'Issued Date:'} {debitNote.date}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2.5">
                    <Button asChild variant="outline" size="sm" className="gap-2">
                        <Link href={`/purchasing/debit-notes/${debitNote.id}/print`}>
                    ...
                    {debitNote.status === 'draft' && (
                        ...
                        >
                            ...
                            {isRtl ? 'ترحيل الإشعار وتخفيض الذمة' : 'Post & Update GL'}
                        </Button>
                    )}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 space-y-1">
                    <span className="text-xs font-medium text-neutral-400 flex items-center gap-1.5">
                        ...
                        {isRtl ? 'بيانات المورد' : 'Vendor'}
                    </span>
                    <div className="text-base font-semibold text-neutral-900 dark:text-neutral-100">
                        {debitNote.vendor?.name || '—'}
                    </div>
                    {debitNote.vendor_bill && (
                        <div className="text-xs text-neutral-500">
                            {isRtl ? 'مرتبط بفاتورة:' : 'Linked Bill:'} <span className="font-mono text-indigo-600">{debitNote.vendor_bill.bill_number}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 space-y-1">
                    ...
                    <div className="text-base font-semibold text-neutral-900 dark:text-neutral-100">
                        {debitNote.branch?.name || (isRtl ? 'المركز الرئيسي' : 'Headquarters')}
                    </div>
                    <div className="text-xs text-neutral-500 truncate" title={debitNote.reason}>
                        {debitNote.reason || (isRtl ? 'مردودات مشتريات عامة' : 'General purchase return')}
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 space-y-1">
                    <span className="text-xs font-medium text-neutral-400 flex items-center gap-1.5">
                        ...
                        {isRtl ? 'إجمالي الإشعار المدين' : 'Total Amount'}
                    </span>
                    <div className="text-xl font-bold font-mono text-indigo-600 dark:text-indigo-400">
                        {parseFloat(debitNote.total).toLocaleString(undefined, { minimumFractionDigits: 2 })} <span className="text-xs font-normal text-neutral-500">{isRtl ? 'ر.س' : 'SAR'}</span>
                    </div>
                    <div className="text-xs text-neutral-500">
                        {isRtl ? 'شامل ضريبة:' : 'Inc. VAT:'} {parseFloat(debitNote.tax_amount).toFixed(2)} SAR
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm">
                <div className="p-4 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
                    <h2 className="text-base font-semibold text-neutral-900 dark:text-neutral-100">
                        {isRtl ? 'تفاصيل الأصناف والكميات المرتجعة للمورد' : 'Returned Items & Quantities'}
                    </h2>
                </div>
                    ...
                    <table className="w-full text-sm text-start">
                        <thead className="bg-neutral-50 dark:bg-neutral-800/60 text-neutral-600 dark:text-neutral-400 font-semibold border-b border-neutral-200 dark:border-neutral-800">
                            <tr>
                                ...
                                <th className="py-3 px-4 text-start">{isRtl ? 'المستودع المصدر' : 'Source Warehouse'}</th>
                                <th className="py-3 px-4 text-start">{isRtl ? 'الوصف' : 'Description'}</th>
                                <th className="py-3 px-4 text-start">{isRtl ? 'الكمية' : 'Qty'}</th>
                                <th className="py-3 px-4 text-start">{isRtl ? 'سعر الوحدة' : 'Unit Cost'}</th>
                            ...
                            {debitNote.lines.map((line, idx) => (
                                <tr key={line.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/40">
                                    ...
                                    <td className="py-3 px-4 text-neutral-600 dark:text-neutral-400">
                                        {line.warehouse?.name || (isRtl ? 'بدون صرف' : 'None')}
                                    </td>
                                    <td className="py-3 px-4 text-neutral-600 dark:text-neutral-400">
                                        {line.description}
                                    </td>
                ...
                <div className="p-4 bg-neutral-50 dark:bg-neutral-800/40 border-t border-neutral-200 dark:border-neutral-800 flex justify-end">
                    <div className="w-full sm:w-72 space-y-2 text-sm">
                        <div className="flex justify-between text-neutral-600 dark:text-neutral-400">
                            ...
                            <span className="font-mono">{parseFloat(debitNote.subtotal).toFixed(2)} SAR</span>
                        </div>
                        <div className="flex justify-between text-neutral-600 dark:text-neutral-400">
                            <span>{isRtl ? 'ضريبة القيمة المضافة:' : 'VAT Amount:'}</span>
                            <span className="font-mono">{parseFloat(debitNote.tax_amount).toFixed(2)} SAR</span>
                        </div>
                        <div className="flex justify-between text-base font-bold text-neutral-900 dark:text-neutral-100 border-t pt-2 dark:border-neutral-700">
                            <span>{isRtl ? 'إجمالي الإشعار المدين:' : 'Net Debited:'}</span>
                            <span className="font-mono text-indigo-600 dark:text-indigo-400">{parseFloat(debitNote.total).toFixed(2)} SAR</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {debitNote.status === 'posted' && debitNote.journal_entry && (
                <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-6 space-y-4">
                    <div className="flex items-center gap-2 text-base font-semibold text-neutral-900 dark:text-neutral-100 border-b pb-3 dark:border-neutral-800">
                        ...
                        <span className="text-xs font-mono font-normal text-neutral-400">({debitNote.journal_entry.entry_number})</span>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs text-start">
                            ...
                            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                                {debitNote.journal_entry.lines.map((jl) => (
                                    <tr key={jl.id}>
