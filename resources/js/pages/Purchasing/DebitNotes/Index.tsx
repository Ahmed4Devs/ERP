...
import { Plus, Search, RotateCcw, CheckCircle2, Clock, Eye, Printer, ArrowLeft, ArrowRight, DollarSign } from 'lucide-react';
...
interface Props {
    debitNotes: PaginatedData<DebitNote>;
    metrics: {
        total_notes: number;
        posted_count: number;
...
export default function DebitNotesIndex({ debitNotes, metrics, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/purchasing/debit-notes', {
    ...
    return (
        <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
            <Head title={isRtl ? 'الإشعارات المدينة ومردودات المشتريات' : 'Purchase Debit Notes'} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100 flex items-center gap-2.5">
                        ...
                        {isRtl ? 'الإشعارات المدينة ومردودات المشتريات' : 'Purchase Debit Notes'}
                    </h1>
                    <p className="text-sm text-neutral-500 mt-1">
                        {isRtl
                            ? 'إدارة مردودات المشتريات للموردين، تخفيض الذمم الدائنة، وعكس ضريبة المدخلات وإخراج البضاعة'
                            : 'Manage vendor returns, liability deductions, input VAT reversals & stock issues'}
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white gap-2 shadow-sm">
                        <Link href="/purchasing/debit-notes/create">
                            ...
                            {isRtl ? 'إشعار مدين جديد' : 'New Debit Note'}
                        </Link>
                    </Button>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-neutral-900 p-5 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-medium uppercase tracking-wider text-neutral-500">
                            {isRtl ? 'إجمالي الإشعارات المدينة' : 'Total Debit Notes'}
                        </span>
                ...
                <div className="bg-white dark:bg-neutral-900 p-5 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-medium uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
                            {isRtl ? 'إجمالي المردودات المرحلة' : 'Total Debited'}
                        </span>
                    ...
                    <div className="mt-3 text-2xl font-bold text-indigo-600 dark:text-indigo-400">
                        {parseFloat(String(metrics.total_debited || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })} <span className="text-sm font-normal text-neutral-500">{isRtl ? 'ر.س' : 'SAR'}</span>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSearch} className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <Input
                        type="text"
                        placeholder={isRtl ? 'بحث برقم الإشعار أو اسم المورد...' : 'Search note number or vendor...'}
                        value={search}
                ...
                <select
                    ...
                    className="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-start">
                        <thead className="bg-neutral-50 dark:bg-neutral-800/60 text-neutral-600 dark:text-neutral-400 font-semibold border-b border-neutral-200 dark:border-neutral-800">
                            <tr>
                                <th className="py-3.5 px-4 text-start">{isRtl ? 'رقم الإشعار المدين' : 'Debit Note #'}</th>
                                <th className="py-3.5 px-4 text-start">{isRtl ? 'المورد' : 'Vendor'}</th>
                            ...
                            {debitNotes.data.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="py-12 text-center text-neutral-400">
                                        ...
                                        {isRtl ? 'لا توجد إشعارات مدينة مسجلة' : 'No debit notes found'}
                                    </td>
                                </tr>
                            ) : (
                                debitNotes.data.map((note) => (
                                    <tr key={note.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/40 transition-colors">
                                        <td className="py-3.5 px-4 font-mono font-medium text-indigo-600 dark:text-indigo-400">
                                            <Link href={`/purchasing/debit-notes/${note.id}`} className="hover:underline">
                                        ...
                                        <td className="py-3.5 px-4 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <Button asChild variant="ghost" size="sm" className="h-8 w-8 p-0">
                                                    <Link href={`/purchasing/debit-notes/${note.id}`} title={isRtl ? 'عرض' : 'View'}>
                                                        <Eye className="h-4 w-4" />
                                                    </Link>
                                                </Button>
                                                <Button asChild variant="ghost" size="sm" className="h-8 w-8 p-0">
                                                    <Link href={`/purchasing/debit-notes/${note.id}/print`} title={isRtl ? 'طباعة' : 'Print'}>
                ...
                {debitNotes.last_page > 1 && (
                    <div className="p-4 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-between text-sm">
                        <span className="text-neutral-500">
                            {isRtl
                                ? `الصفحة ${debitNotes.current_page} من ${debitNotes.last_page}`
                                : `Page ${debitNotes.current_page} of ${debitNotes.last_page}`}
                        </span>
                        <div className="flex items-center gap-2">
                            {debitNotes.current_page > 1 && (
                                <Button asChild variant="outline" size="sm">
                                    <Link href={`/purchasing/debit-notes?page=${debitNotes.current_page - 1}&search=${search}&status=${status}`}>
                                        {isRtl ? <ArrowRight className="h-4 w-4" /> : <ArrowLeft className="h-4 w-4" />}
                                        {isRtl ? 'السابق' : 'Previous'}
                                    </Link>
                                </Button>
                            )}
                            {debitNotes.current_page < debitNotes.last_page && (
                                <Button asChild variant="outline" size="sm">
                                    <Link href={`/purchasing/debit-notes?page=${debitNotes.current_page + 1}&search=${search}&status=${status}`}>
                                        {isRtl ? 'التالي' : 'Next'}
